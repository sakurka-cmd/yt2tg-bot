...
| `YTDLP_COOKIES` | Нет | Путь к cookies.txt (Netscape) от залогиненного YouTube-аккаунта — для возрастных видео (по умолчанию пусто) |
...
### Очередь скачивания (2026-09-07)

Все загрузки (разовые /dl, плейлисты, бэкфилл, планировщик подписок) идут через
единую очередь `bot/dl_queue.py` — одновременно качается только одно видео (FIFO).

Зачем: перед каждой загрузкой `download_video()` чистит устаревшие `*.part` /
`*.temp.*` файлы в TMP_DIR. Это безопасно только при одной активной загрузке.
Раньше несколько ссылок подряд качались параллельно — новая загрузка удаляла
недокачанные файлы предыдущей, и выживала только последняя ссылка.

Поведение:
- Ссылки отправленные подряд (или несколько ссылок в одном сообщении — бот
  спросит качество один раз) встают в очередь и качаются по очереди.
- Повторный запрос того же видео, пока оно в очереди — «уже скачивается»
  (sentinel DUPLICATE), дубликат не создаётся.
- /status показывает активную загрузку и список очереди.
- /cancel убирает из очереди ещё не начавшиеся загрузки пользователя
  (активная yt-dlp загрузка не прерывается).
- Сентинелы download_video(): DUPLICATE (уже в очереди), CANCELLED
  (убрано через /cancel пока ждало очереди).
- Планировщик подписок при DUPLICATE пропускает видео и проверит его
  на следующем цикле (не помечает обработанным).

## Cookies / возрастные видео (2026-09-08)

Видео с возрастным ограничением (например, клипы Mylène Farmer) YouTube не
отдаёт без авторизации: yt-dlp отвечает «Sign in to confirm your age».
Никакой player client / имперсонация это ограничение не обходит — нужен
залогиненный аккаунт, т.е. cookies.

Без cookies бот честно сообщает: «Видео с возрастным ограничением…»
(sentinel `AGE_RESTRICTED`) и помечает видео, чтобы не retry'ить его каждый цикл.

Настройка (по желанию, только если нужны возрастные видео):

1. В браузере, где вы залогинены в YouTube, поставьте расширение
   «Get cookies.txt LOCALLY» (Chrome/Firefox) и экспортируйте cookies для
   youtube.com в формате Netscape.
2. Загрузите файл на сервер: `/opt/yt2tg-bot/cookies.txt`, права `chmod 600`.
3. В `/opt/yt2tg-bot/.env` добавьте:
   `YTDLP_COOKIES=/opt/yt2tg-bot/cookies.txt`
4. `systemctl restart yt2tg-bot`.

Cookies передаются во ВСЕ вызовы yt-dlp (инфо, скачивание, субтитры),
так что разовые /dl и подписки ведут себя одинаково. Файл в `.gitignore`
не попадает — не храните его в git. Если YouTube начнёт блокировать
аккаунт-донор — просто удалите переменную и перезапустите бота.
