...
async def upload_video(file_path: str, title: str,
    ...
    duration: optional video duration in seconds (from yt-dlp info["duration"]).
    Passed via x-duration header so the APK can show it on the thumbnail badge.
    """
    ...
    logger.info("Uploading to VideoHost: %s (%.1f MB) playlist=%s published_at=%s yt_id=%s thumb=%s subs=%s duration=%s",
    ...
    try:
        ...
        if duration is not None and duration > 0:
            # yt-dlp returns duration in seconds as a float. The backend stores
            # it in Video.duration (Float?) and the APK shows it on the thumbnail.
            upload_headers["x-duration"] = str(duration)
        if thumb_fs_path:
